...
using namespace std;
//нужны следующие лбходы левый и правый ( обходы деревьев)


//постановка задачи (кратчайший остов)

//построить дерево для заданной предметной области--курсавая
//экспертная система

//эвристический поиск  (if then elses)
    ...
    for (int i = 0; i < path.size() - 1; i+=2) {
        if(i>1) 
        {
            if(path[i] != path[i-2]){
                ...
                {
                    auto it = find(wait.begin(), wait.end(), path[i]);
                    wait.erase(it);
                    a--;
                }
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 9: {
                ...
                bool a = work->find_path_fw(u,v,paths,path);
                if(a==0)
                {cout<< "Нет путей\n";
                    break;
                }
